    ...
    setTabletLocations(state, payload) {
        ...
        for (let i = 0; i < tabletLocations.length; i++) {
            for (let j = 0; j < tabletLocations[i].classrooms.length; j++) {
                const classroomName = tabletLocations[i].classrooms[j].name.replace(/Lecture room/ig, "").trim();
                tabletLocations[i].classrooms[j].name = classroomName;
            }
            tabletLocations[i].classrooms.sort((a, b) => a.name.localeCompare(b.name, undefined, {numeric: true}));
